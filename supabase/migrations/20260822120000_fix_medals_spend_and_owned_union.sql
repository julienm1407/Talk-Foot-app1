-- Médailles : ne plus utiliser GREATEST (les dépenses étaient annulées au merge).
-- Inventaire boutique : conserve l'union ownedItemIds (idempotent si déjà déployé).
...
declare
  ...
  v_merged_tokens int;
  v_existing_medals int := greatest(0, coalesce((v_existing_wallet ->> 'medals')::int, 0));
  v_incoming_medals int := greatest(0, coalesce((v_incoming_wallet ->> 'medals')::int, 0));
  v_merged_medals int;
  v_existing_won int := 0;
  v_incoming_won int := 0;
  v_elem jsonb;
  v_existing_owned jsonb := coalesce(v_existing -> 'profile' -> 'ownedItemIds', '[]'::jsonb);
  v_incoming_owned jsonb := coalesce(v_incoming -> 'profile' -> 'ownedItemIds', '[]'::jsonb);
  v_merged_owned jsonb;
  v_existing_prem_owned jsonb := coalesce(
    v_existing -> 'profile' -> 'premiumInventory' -> 'ownedItemIds',
    '[]'::jsonb
  );
  v_incoming_prem_owned jsonb := coalesce(
    v_incoming -> 'profile' -> 'premiumInventory' -> 'ownedItemIds',
    '[]'::jsonb
  );
  v_merged_prem_owned jsonb;
begin
  ...
  -- Médailles : trust le client sauf réinit à 0 face à un solde cloud > 0.
  if v_incoming_medals = 0 and v_existing_medals > 0 then
    v_merged_medals := v_existing_medals;
  elsif v_incoming_medals < v_existing_medals then
    v_merged_medals := v_incoming_medals;
  else
    v_merged_medals := v_incoming_medals;
  end if;
  ...
  if jsonb_typeof(v_existing_owned) <> 'array' then
    v_existing_owned := '[]'::jsonb;
  end if;
  if jsonb_typeof(v_incoming_owned) <> 'array' then
    v_incoming_owned := '[]'::jsonb;
  end if;
  ...
  from (
    ...
    from (
      select value #>> '{}' as elem from jsonb_array_elements(v_existing_owned)
      union
      select value #>> '{}' as elem from jsonb_array_elements(v_incoming_owned)
    ) u
    where elem is not null and length(trim(elem)) > 0
  ) x;

  v_result := jsonb_set(v_result, '{profile,ownedItemIds}', v_merged_owned, true);

  if jsonb_typeof(v_existing_prem_owned) <> 'array' then
    v_existing_prem_owned := '[]'::jsonb;
  end if;
  if jsonb_typeof(v_incoming_prem_owned) <> 'array' then
    v_incoming_prem_owned := '[]'::jsonb;
  end if;

  select coalesce(jsonb_agg(to_jsonb(x.id) order by x.id), '[]'::jsonb)
  into v_merged_prem_owned
  from (
    select distinct elem as id
    from (
      select value #>> '{}' as elem from jsonb_array_elements(v_existing_prem_owned)
      union
      select value #>> '{}' as elem from jsonb_array_elements(v_incoming_prem_owned)
    ) u
    where elem is not null and length(trim(elem)) > 0
  ) x;

  if (v_result -> 'profile' -> 'premiumInventory') is null
     or jsonb_typeof(v_result -> 'profile' -> 'premiumInventory') <> 'object' then
    v_result := jsonb_set(
      v_result,
      '{profile,premiumInventory}',
      jsonb_build_object('ownedItemIds', v_merged_prem_owned, 'equippedByCategory', '{}'::jsonb),
      true
    );
  else
    v_result := jsonb_set(
...
comment on function public.talkfoot_merge_client_app_state is
  'Fusion app_state : wallet (dépenses médailles/jetons), paris, avatar, union ownedItemIds boutique.';
