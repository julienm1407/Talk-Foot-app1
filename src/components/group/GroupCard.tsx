...
function ThemeBackdrop({ group }: { group: SupporterGroup }) {
  return (
    <div
...
function ActiveSupportersFacepile({ size }: { size: 'md' | 'sm' }) {
  const avatarCls = size === 'sm' ? 'size-6 ring-2 ring-white' : 'size-7 ring-2 ring-white'
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={cn('font-semibold text-slate-500', labelCls)}>Soutiens en ligne</span>
    </div>
  )
}
...
export function GroupCard({
  ...
  const shell = cn(
    ...
    railDense ? 'rounded-2xl px-2.5 py-2' : rail ? 'px-3 py-3' : encart ? 'px-4 py-4' : 'px-5 py-4',
    className,
  )
    ...
    <div className={shell} style={themeStyle}>
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {preview && encart ? (
            <div className="mt-2 rounded-xl border border-slate-200/60 bg-white/50 px-2.5 py-1.5">
              <span className="text-[9px] font-black uppercase tracking-wide text-slate-400">Dernier message</span>
              <p className="mt-0.5 line-clamp-2 text-[11px] font-semibold text-slate-600">{preview}</p>
            </div>
          ) : null}
        </div>
