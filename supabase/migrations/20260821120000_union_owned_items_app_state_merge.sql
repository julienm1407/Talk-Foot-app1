-- Préserve l'inventaire boutique (ownedItemIds) : union serveur ∪ client.
-- Évite qu'un appareil périmé écrase un maillot acheté ailleurs.
...
begin
  ...
  v_merged_wallet := jsonb_build_object(
    ...
    'medals', greatest(
      0,
      coalesce((v_existing_wallet ->> 'medals')::int, 0),
      coalesce((v_incoming_wallet ->> 'medals')::int, 0)
    )
  );
  ...
  -- Inventaire boutique : union des ids (jamais perdre un achat cloud).
...
comment on function public.talkfoot_merge_client_app_state is
  'Fusion app_state : wallet (jetons/médailles), paris, avatar, et union ownedItemIds boutique.';
