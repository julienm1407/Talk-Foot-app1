...
export function useGlobalBetSettlement() {
  ...
  useEffect(() => {
    ...
    const settleMatch = async (match: Match, currentBets: Bet[]) => {
      ...
      if (!openOnMatch.length) return { bets: currentBets, tokenDelta: 0 }
      ...
      return { bets: scorerResult.bets, tokenDelta: tokenDelta + scorerResult.tokenDelta }
    }
  ...
  }, [matches, bets, setBets, patchWallet, cloud, betTokenMultiplier])
}
