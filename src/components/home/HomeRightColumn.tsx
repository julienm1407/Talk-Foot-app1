        ...
        <ul className="mt-3 space-y-2" role="list">
          {groups.map((g) => (
            <li key={g.id}>
              <Link
                to={`/group/${g.id}`}
                className="flex items-center justify-between gap-1.5 rounded-xl border border-tf-grey-pastel/40 bg-white/80 px-2 py-2 transition hover:border-tf-electric/25 hover:shadow-sm"
              >
                <span className="min-w-0 truncate text-xs font-black text-tf-dark">{g.name}</span>
                <span className="shrink-0 rounded-full bg-rose-50 px-1.5 py-0.5 text-[9px] font-black text-rose-700">
                  🔥{g.onlineNow ?? '—'}
                </span>
              </Link>
            </li>
