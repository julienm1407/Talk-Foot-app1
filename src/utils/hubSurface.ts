...
export function hubPillLink(appearance: Appearance, size: 'xs' | 'sm' | 'md' = 'sm') {
  ...
  return cn(
    ...
    sz,
    L
      ? 'border-tf-dark/16 bg-white/92 text-tf-dark hover:border-tf-dark/26 hover:bg-white'
      : 'border-white/18 bg-white/[0.09] text-sky-100 hover:border-white/30 hover:bg-white/[0.14]',
  )
}
